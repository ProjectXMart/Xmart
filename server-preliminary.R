server = function(input, output,session) {
  ...
  observeEvent(input$button, {
    ...
    if (input$inputId == "Single") {
      ...
      #   map_key <- "AIzaSyD6AfICWEZMcI40_Alta3JTLlHD-im2LRk"
      #   set_key(key =map_key)
      #   google_map(data = x, key = map_key) %>%
      #     #add_markers(lat = "Lat", lon = "Lng", info_window = "FSA",marker_icon = )%>%
      #    
      #  
      # })
    }
    ...
    show("panel")
     ...
     #tradearea = read.csv("C:/Users/ali518/Desktop/X-Mart/tradearea_new.csv")
